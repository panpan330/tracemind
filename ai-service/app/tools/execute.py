...
def execute_tool(tool_name: str, incident_id: int | None = None,
    ...
    # tool_call_id 无 mcp_invocation_id 时生成唯一 id(call_tool 端点 agent_run_id=0 下
    # 固定 "legacy-direct" 会撞 tool_call 唯一约束 uk_tool_call_agent_toolcall)
    ctx = ClientInvocationContext(incident_id=incident_id or 0,
                                  agent_run_id=agent_run_id or 0,
                                  tool_call_id=mcp_invocation_id or f"legacy-{uuid.uuid4().hex[:8]}",
