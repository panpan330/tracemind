...
class McpHttpTransport:
    ...
    async def _call_with_retry(self, name: str, params: dict,
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                return self._parse(name, result, ctx, attempt, attempt_id)
            except ClientError as e:
                last_error = e
                if not e.retryable:
                    raise
            except Exception as e:  # noqa: BLE001
                last_error = ClientError(MCP_DISCONNECTED, f"{name}: {e}", retryable=True)
    ...
    @staticmethod
    def _parse(name: str, result, ctx: ClientInvocationContext,
               attempt: int, attempt_id: str) -> dict:
        text = "".join(getattr(c, "text", "") or "" for c in result.content or []
                       if getattr(c, "type", "") == "text")
        try:
            out = json.loads(text)
        except Exception as e:  # noqa: BLE001
            raise ClientError(MCP_DISCONNECTED, f"{name}: 响应非 JSON: {e}", retryable=True) from e
