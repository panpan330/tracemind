...
**确定性降级组件**(运行时不用 FakeLLM 伪装):`TemplateHypothesisGenerator`(假设)、`DeterministicEvidencePlanner`(按缺失 E1~E5 确定性选工具)、`TemplatePostmortemRenderer`(报告)。propose_fix 本身不受模型影响。面试话术:**FakeLLM 仅用于测试、CI 和显式回归模式;它不会成为 real_strict 或 real_demo 的运行时降级对象**——运行时降级是确定性程序,不伪装成模型正常工作。
...
- **调查启动后模型失效**:real_strict → `needs_human`;real_demo → 确定性组件;均记录 ModelCall + SSE 事件。
...
程序按**独立资格判断**计算,每轮把所有满足条件的工具暴露给 LLM 选择(不退化为固定顺序):
- get_service_metrics:E1 缺失即可调用;
- get_trace:E2 缺失且已有合法 representative_trace_id;
- list_expensive_query_digests:E3 缺失即可调用;
- get_query_plan:E4 缺失且已有 INVENTORY_LOOKUP query_ref;
- get_index_info:E5 缺失即可调用。

多候选时全部提供,LLM 决定优先级(仍为真实 Tool Calling)。确定性降级规划器仍按 E1→E2→E3→E4→E5 顺序执行,两者不冲突。
...
- payload:`doc_id/title/section_path/chunk_index/fault_category/service/scenario_id/version/source_path/content_hash/embedding_model/embedding_dimensions/enabled/environment(common|demo|production)`;
...
- **RAG 失败业务行为**:检索失败 → `retrieval_record(status=failed)` + `agent_run.rag_degraded=true` + SSE `rag_degraded` → hypothesize 用**无知识上下文的真实 LLM 继续**,不进入 needs_human;
- `TRACEMIND_RAG_MODE = off | optional | required`:off = RAG 关闭对照组(不算降级);optional = real_demo 默认(失败允许继续);required = 正式全栈 RAG 验收(该次 `rag_degraded=true` 即失败)。
...
**离线评测 16 条(固定)**:E1~E5 完整不同描述正例 ×4;分别缺 E1/E2/E3/E4/E5 ×5;网络/配置/流量负例 ×3;索引存在但接口慢 ×1;调查工具超时 ×1;证据矛盾 ×1;**下游服务超时 ×1**。
- **工具安全回归测试集**(独立,不进 16 条):ScriptedLLM 固定返回非法 tool calls(未知工具/多工具/非法参数/尝试 execute_fix),验证 `unsafe_tool_execution_count == 0`;刻意注入的非法响应**不计入**模型质量指标,`模型非法决策率 ≤5%` 仅用于真实模型自然输出。
...
- **不改动**:E1~E5 根因闸门判定逻辑、七工具层实现、三连接池与四账号、审批中断、恢复判定;
- **兼容扩展**:SSE 协议与现有事件兼容,新增 `llm_degraded/rag_degraded/rag_recovered` 事件;Vue 三个页面结构不变,增加降级横幅、模型信息、知识引用展示(小规模兼容扩展,非重做前端);
