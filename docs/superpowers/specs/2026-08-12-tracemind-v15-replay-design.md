...
> 分段评审定稿:方案 A(调查时快照 + 只读 API + 前端本地播放),三批共 24 条评审意见全部采纳。
...
round_no INT NULL, attempt_no INT DEFAULT 1, step_type, step_title, step_status,
sequence_no, state_before_json, state_after_json, decision_json, operation_json,
source_references_json, actual_duration_ms,
replay_schema_version, policy_version, prompt_version, tool_contract_version,
normalization_rule_version, snapshot_hash, payload_size_bytes, occurred_at
UNIQUE(agent_run_id, sequence_no);排序 agent_run_id → sequence_no → id
```

- **两段式 = 追加**:同一逻辑步骤多条 phase 记录(`logical_step_id` 相同):`started`(含 state_before)→ `completed/failed`(含 state_after)。Projector 按 `logical_step_id` 聚合为 UI 一个步骤。
- **sequence_no 原子分配**:`agent_run` 维护 `next_replay_sequence`,每次写入原子自增获取;冲突仅有限重试,绝不覆盖已有记录(审批 API 与 Agent 执行可能并发)。
- 大型工具结果**不塞进快照**:仅经 source_references 引用;限单步 payload(`payload_size_bytes`)。

### 3.2 专用 DTO `ReplayStateSnapshot`(不序列化完整 IncidentState)
...
快照生成流程(ReplaySnapshotFactory):字段白名单 → 深拷贝 → 脱敏 → 确定性排序(hypotheses 按 ID、facts/policies 按 code、evidence 引用按 ID)→ JSON 序列化 → 稳定 `snapshot_hash`(便于测试与完整性校验)。

### 3.3 Step 覆盖(按实际执行写入,非预设固定序列)
...
- **外部操作**(LLM/MCP/Prometheus/Jaeger 查询/KILL CONNECTION/LangGraph Checkpoint/Java 请求)→ 无法与 MySQL 回放记录同事务:提交 `started` → 执行外部操作 → 提交 `completed/failed`。
  ...
  - 外部操作成功但进程崩溃 → 回放显示"已开始但缺完成记录"(`replay_status=partial`),经 `fix_execution` 审计引用展示"外部操作可能已执行",**绝不反向生成虚假 completed**。
- **诚实边界**:Step 写入失败时**尽最大可能记录捕获错误**;`replay_status=partial` 由运行结束/读取时的完整性检查标记;数据库完全不可用时仅保留应用错误日志,不承诺写入 partial。

### 3.5 Run 级版本冻结

以下版本在 Agent Run 启动时确定并冻结,Step 冗余保存但值来自 Run 启动元数据(不能执行到一半读到新版本):

```
replay_schema_version / policy_version / tool_contract_version / normalization_rule_version
```

仅发生变化的节点才需要对应版本(非 LLM 节点 `prompt_version` 可空)。

### 3.6 完整性检查 → replay_status

Run 结束后执行一次 Replay 完整性检查,API 读取时做轻量校验:

- `complete`:全部步骤有 completed/failed,引用有效
- `partial`:`started` 无对应完成记录 / sequence_no 异常 / source_reference 指向不存在的业务记录 / 必需终态步骤缺失 / 快照 Schema 无法解析 / Hash 校验失败
- `partial_legacy`:V1.5 之前创建的 Incident(无完整快照),仅从已有审计投影可恢复部分步骤
- `unsupported`:快照 Schema 不兼容
- 无 Replay 数据 → 返回 `unavailable`(不是空完整回放)

## 4. Replay Projector(投影层)

- **聚合阶段记录**:同 `logical_step_id` 的多条 phase 聚合为一个 UI 步骤,输出:

```json
{ "stepIndex": 3, "logicalStepId": "step_xxx", "sourceSequenceNos": [10, 11], "stepStatus": "completed" }
```
...
- **partial 占位**:started 无 completed/failed → 输出 `stepStatus=incomplete` + `missingParts=["stateAfter","operationResult"]`,时间轴**显式缺失节点**,不跳过(避免观众误以为时间线完整)。
...
GET /api/incidents/{incident_id}/replay
    → Incident 级 Manifest:replay_status / schema 版本 / Run 列表 / defaultRunId
...
GET /api/incidents/{incident_id}/replay/runs/{agent_run_id}/steps/{step_index}
    → 单步技术详情(懒加载)
```

- **归属校验**:`agent_run.incident_id == URL 的 incident_id`,防替换 Run ID 跨 Incident 读。
- **defaultRunId**:最新的**已终止** Run(不是简单取最大 ID)。
- Manifest 字段:

```json
{
  "replayId": "replay_xxx", "replayStatus": "complete|partial|partial_legacy|in_progress|unsupported|unavailable",
  "sourceReplaySchemaVersion": "1.0", "responseSchemaVersion": "1.0", "playbackPolicyVersion": "1",
  "totalSteps": 18, "keyStepIndexes": { "diagnosis": 7, "approval": 9, "execution": 11, "recovery": 13 },
  "supportedSpeeds": [1, 2, 4]
}
```

  - `keyStepIndexes` 引用 **stepIndex**;关键节点不存在时不返回该字段,前端禁用对应跳转按钮。
- **/steps 一次返回播放必需数据**:`stateBefore / stateAfter / decisionSummary / operationSummary / sourceReferenceSummary / actualDurationMs / displayDurationMs`——加载完成后断网核心播放仍继续。单步详情接口只懒加载:脱敏参数详情 / MCP 调用信息 / PromQL 模板信息 / Trace 与审计引用 / 版本信息 / 较大规范化结果。
...
详情页 → "查看历史回放" → 加载 Manifest 与 steps → **默认暂停在第 0 步**(不自动播放,防错过开头)→ 用户点播放。

URL:`/replay?runId=123&step=8`;缺 `runId` 用 defaultRunId;`step` 越界修正到有效范围;Run 切换回第 0 步并暂停;浏览器前进/后退恢复 Run+Step;**不把播放/暂停状态写入 URL**。

### 6.2 布局

- 顶部:步骤时间轴(实际节点序列;partial 缺失节点显式标记,不用动画/文字补全)
- 左侧:状态快照——假设(proposed/supported/refuted/confirmed)+ 共享 Facts(unknown/supported/refuted/stale/conflict)+ 双 DiagnosticPolicy + 安全排他条件(分区展示,不与根因证据混)+ 当前根因判定
- 右侧:本步详情——**执行前摘要 → 决策与操作 → 执行结果 → 状态变化**(见 6.4)
...
- 初始 IDLE:显示第 0 步的 `stateBefore`
- 选中第 N 步:左侧显示该步 `stateAfter`,高亮标记 `stateBefore → stateAfter` 变化的字段(避免分不清"执行前/后")
- 右侧依次:执行前状态摘要 → 决策与操作 → 执行结果 → 状态变化

### 6.5 颜色语义(颜色之外保留文字+图标)
...
播放、暂停、上一步/下一步正常;1×/2×/4× 正确;可跳转任意步骤与关键节点(缺失节点按钮禁用);手动跳转后自动暂停;页面隐藏自动暂停;刷新后经 `?runId=&step=` 恢复;完成后可重新播放;同一页面无重复计时器;partial 回放显式展示缺失事件(incomplete 占位)。

### 只读无副作用(证明回放真无副作用)

- 播放期间网络面板**只允许 Replay 相关 GET 请求**;无 Incident 创建、审批、执行、LLM 或 MCP 请求
