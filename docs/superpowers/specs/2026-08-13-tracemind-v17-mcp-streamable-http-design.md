...
前置:V1.2(MCP stdio 工具层协议化)→ V1.3(多场景)→ V1.4(真实观测)→ V1.5(回放)→ V1.6(迁移器/Run Profile/本地与 VM 验收能力;原计划的 GitHub Actions CI 因个人项目执行时间与维护成本主动取消,GitHub 仅保留远程仓库职责)

## 1. 背景与目标

V1.2 起 MCP Server 由 AI 服务 Spawn 子进程、经 stdio 调用,受"必须与 AI 服务同机"限制。V1.7 将其升级为**独立远程工具服务**:

- 独立进程、独立容器、独立凭据、独立镜像与部署生命周期;涉及 Tool Contract / InvocationContext / 协议版本变化时采用**协调升级与回滚**(未定义 N-1 兼容矩阵前,不声称可任意单独回滚 MCP Server);
...
- **vm_release/production Profile 禁止 Spawn MCP stdio 子进程**,启动时断言 `transport == streamable_http` 且无 stdio 子进程;标准 Profile 集:`local / offline_eval / vm_smoke / vm_release / production`(V1.6 的 `full_e2e` 更名为 `vm_release`,语义对齐"VM 发布验收")。
...
- 冻结版本常量进 `app/mcp/contract.py`,契约校验扩展:
  - 协议版本:由代码/Contract 定义 `SUPPORTED_MCP_PROTOCOL_VERSIONS`,可选部署约束 `REQUIRED_MCP_PROTOCOL_VERSION`(只能从支持列表收紧,不能通过环境变量声称支持 SDK 实际不支持的版本),初始化协商得到 `negotiated_protocol_version`;`MCP_SDK_VERSION` 由 `importlib.metadata` 读取实际安装版本,与 Build Manifest 不一致则启动失败,**不作为可修改环境配置**(环境变量无法改变已安装 SDK,只能伪造字符串);
...
- 入口:
  - `python -m app.mcp.server_stdio`(本地/评测,现状保留);
  - `python -m app.mcp.server_http`(独立容器,`stateless_http=True`;内部 `create_mcp_server(runtime)` + 安全中间件链)。
- 每次测试创建新实例,避免 ToolRegistry / Fixture / 限流状态在测试间污染。
- **Fixture 通过依赖注入创建**:`create_real_tool_runtime()` / `create_fixture_tool_runtime(fixture_file)`;stdio 离线评测可建 Fixture Runtime;HTTP 协议集成测试用 Test Profile 建 Fixture;vm_release/production 只能 Real Runtime,检测到 Fixture 配置直接拒绝启动;标准 HTTP 镜像不包含 Fixture 文件;审计标记 `fixture=false`。
...
- **Client 只注入**(受控 Header,**逐请求生成**,不存共享 Client Header):`X-TraceMind-Incident-Id`、`X-TraceMind-Agent-Run-Id`、`X-TraceMind-Tool-Call-Id`、`X-TraceMind-Purpose`、`X-TraceMind-Context-Version`;`traceparent`/`tracestate` 为 W3C Transport Trace Context Header。
...
- 区分 **Logical Tool Call**(`tool_call_id`,重试保持不变)、**Attempt**(`attempt_no`,每次重试决策递增;**由 Server 原子分配**,不信任请求端)与 **HTTP 请求重传**(`client_attempt_id`:同一次 HTTP 请求重传保持相同 id、一次重试决策生成新 id;`mcp_request_id`:每个服务端接收的协议请求唯一)。幂等:`UNIQUE(tool_call_pk, client_attempt_id)`;遇重复 `client_attempt_id`:已完成 → 返回先前受控结果/重复结果引用;执行中 → `ATTEMPT_IN_PROGRESS`;状态未知 → `ATTEMPT_OUTCOME_UNKNOWN`;**不盲目再次执行**(避免重复审计/重复预算/结果时间漂移)。
...
- 配置项:
  ...
  - `TRACEMIND_MCP_HTTP_URL` / `TRACEMIND_MCP_HTTP_BEARER_TOKEN`(Client)/ `TRACEMIND_MCP_HTTP_CONNECT_TIMEOUT_SECONDS` / `TRACEMIND_MCP_HTTP_REQUEST_TIMEOUT_SECONDS` / `TRACEMIND_MCP_HTTP_MAX_RETRIES`
  - `TRACEMIND_MCP_AUTH_CLIENTS_FILE`(Server 侧)
  - `TRACEMIND_MCP_MAX_REQUEST_BYTES=262144` / `TRACEMIND_MCP_MAX_RESULT_BYTES=1048576`
  - 协议版本:**不是环境变量**(见 §4)
- 校验规则:stdio 需要 Server Command,不需要 URL/Token;streamable_http 需要 URL + 凭据;`vm_release`/`production` **禁止选择 stdio**;跨主机 URL 禁止明文 HTTP;HTTP 配置错误启动失败;**不允许自动切换 Transport**。
...
| 方 | 记录与权限 |
|---|---|
| AI Service | 拥有并 INSERT/UPDATE `tool_call`(LLM 选择/逻辑 tool_call_id/Agent Round/业务参数摘要/Evidence 如何消费);**创建后先提交事务,再发送 MCP 请求**;消费成功结果创建 Evidence |
| MCP Server | SELECT `tool_call`;INSERT/UPDATE `tool_call_attempt`(认证结果/HTTP/MCP 请求/Attempt/Transport/工具耗时/Tool Result/错误/Server Instance+Trace ID);INSERT `observation_query`;**不修改 AI 的工具选择/Agent Round/Evidence 消费状态字段** |

表结构与唯一约束:

- `tool_call`:逻辑工具调用。`tool_call.id`(DB 主键)+ `tool_call.tool_call_id`(业务 ID);`UNIQUE(agent_run_id, tool_call_id)`
- `tool_call_attempt`:传输与执行尝试。`tool_call_pk`(FK → tool_call.id);`UNIQUE(tool_call_pk, attempt_no)`、`UNIQUE(tool_call_pk, client_attempt_id)`、`UNIQUE(mcp_request_id)`;字段含 `started_at / completed_at / outcome / protocol_version / transport / server_instance_id / trace_id / request_hash / result_hash / error_code / retryable / latency_ms`
- **attempt_no 原子分配**:`SELECT ... FOR UPDATE` 或逻辑调用维护 `next_attempt_no` 原子自增,**禁止 `SELECT MAX(attempt_no)+1`**(并发下撞唯一约束)
...
- `mcp_tool_auditor` 权限:SELECT incident / agent_run / tool_call;INSERT/UPDATE tool_call_attempt;INSERT observation_query;**不授予整个 `tool_call` 的 UPDATE**(除非确实需要更新少量 Server Aggregate 字段,按需单独授权);禁止修改 incident / hypothesis / evidence / approval / fix_execution;不允许 DDL / KILL;不能读取 LLM 调用审计中的敏感内容。
...
```
agent-mcp-network        internal: true    ai-service, mcp-tools
control-data-network     internal: true    ai-service, mcp-tools, mysql
tool-observation-network internal: true    mcp-tools, mysql, prometheus, jaeger, order-service, inventory-service
llm-egress-network       (非 internal)     仅 ai-service
```

- 标准 HTTP 模式:AI Service 不加入 tool-observation-network;mcp-tools 只加入三个内部网络,**不加入 llm-egress-network**、不映射宿主机端口;Compose Health Check 在容器内部访问 `/health/ready`;禁用隐式 Default Network。
- **边界说明**:普通 Compose Network 隔离服务间访问,不等同于严格的生产级网络防火墙;云防火墙与专用 Egress Gateway 属范围外,V1.7 在 Compose 能力范围内完成隔离。
- **如实说明**:控制库与业务库仍在同一 MySQL 实例,网络无法隔离同实例内的 Schema,最终权限仍由数据库账号保证。

### 10.3 部署顺序与回滚原则

- 部署顺序:数据库追加式 Migration → 部署 mcp-tools → MCP 协议与契约探针 → 部署 ai-service → VM Smoke。
- 回滚原则:数据库 Migration 向后兼容,不立即删除旧字段;AI Client 启动时校验 Server Contract;Contract 不兼容时 Fail-closed,不尝试 Direct/stdio;未实现 N-1 兼容前,不声称可任意单独回滚 MCP Server。
...
- Java 全量测试可选;Vue 全量测试/构建可选;**Vue typecheck + Replay Transport targeted test 必须进入 Local Fast**(V1.7 新增 `mcp_streamable_http` 会影响 Replay API 与 Vue Transport 枚举)。
...
- **额度/模型错误处理**:短期限流(429)按既有退避策略处理;额度耗尽 → 停止验收并保留部分报告;密钥失效 → 修复凭据后重新执行;**不自动更换模型**;人工更换模型必须作为一次新的 Validation Run,并在报告中记录新模型。遇额度问题立即告知用户。
...
**Release 前绑定断言**(创建 V1.7 Tag 前必须通过):工作树干净;报告 Git SHA == 当前 HEAD;两个镜像 label Git SHA == 当前 HEAD;镜像 Digest 已写入报告;Tag 指向的 Commit == 已验收 Commit(避免"验收了 A 代码,却给 B 代码打 Tag")。
